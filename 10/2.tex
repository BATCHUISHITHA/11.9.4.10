...
\begin{table}[!ht]
    \centering
        \input{10/tables/table2}
    \caption{input parameters}
    \label{}
\end{table}

\begin{align}
S_n &= \sum_{n=0}^{n-1} x(n) \\
&=\sum_{n=0}^{n-1} (2n-1)^2 \\
&=\sum _{n=0}^{n-1} 4n^2+1-4n \\
&=\frac{4 (n-1)n(2n-1)}{6} + n -\frac{4 (n-1)n}{2} \\
\because 1^2 + 2^2 + 3^2 + \ldots + n^2 = \frac{n \cdot (n + 1) \cdot (2n + 1)}{6}\\
\because 1 + 2 + 3 + \ldots + n = \frac{n \cdot (n + 1)}{2}\\
&=\frac{2 n(n-1)(2n-1)}{3} + n - 2(n)(n-1) \\
(\text{taking} 
\frac{ 2 n (n-1)}{3}
\text{ common from first and third term;})\\
&=\frac{2 n(n-1) }{3}(2n-1-3)+ n \\
&=\frac{2 n(n-1) }{3}2(n-2) + n \\
&=\frac{ 4n(n-1)(n-2)}{3} + n \\
(\text{ taking }
\frac{n}{3}
\text{ common from both terms; }) \\
&=\frac{n(4(n^2-3n+2)+3)}{3} \\
...
&= U(z) + 8  z \frac{d(U(z))}{dz} + 4z^{2}\frac{d^2(U(z))}{dz^{2}} \\
&= \frac{(1-z^{-1})^2 -8z^{-1}(2-z^{-1})}{(1-z^{-1})^3}
\end{align}

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.70]{10/figs/fig2.png}
    \caption{ }
    \label{}
\end{figure}
